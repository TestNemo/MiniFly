...
static rt_tick_t dispalyBatTime;
static rt_tick_t getMFMsgTime;
...
void main_ui(void)
{
	...
	if(rt_tick_get() >= dispalyBatTime+1000)/*遥控器电量1000ms更新一次*/
	{
//		dispalyBatTime = xTaskGetTickCount();
		dispalyBatTime = rt_tick_get();
	...
	if(rt_tick_get() >= getMFMsgTime+500)
	{
//		getMFMsgTime = xTaskGetTickCount();
		getMFMsgTime = rt_tick_get();
