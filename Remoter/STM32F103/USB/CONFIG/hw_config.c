...
struct rt_messagequeue usbDataDelivery;
static uint8_t msg_pool[128];
...
//	}
    ...
    rt_err_t result;
	result = rt_mq_send(&usbDataDelivery, &data_buffer[0], sizeof(uint8_t) * Nb_bytes);
    if ( result == -RT_EFULL)
...
void usb_vcp_init(void)
{
    ...
    /*init message queue */
    result =  rt_mq_init(&usbDataDelivery, "usbDataDelivery",
                         &msg_pool[0], 
                         128- sizeof(void*), 
                         sizeof(msg_pool), 
                         RT_IPC_FLAG_FIFO);
...
int usbGetDataWithTimout(uint8_t *c)
{
	...
	if (rt_mq_recv(&usbDataDelivery, &msg_pool[0], sizeof(msg_pool), 1000) == RT_EOK)
	{
	    c = msg_pool;
		return 1;
	}
